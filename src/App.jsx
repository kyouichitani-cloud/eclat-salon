...
export default function App(){
  ...
  useLayoutEffect(()=>{
    ...
    const ctx=gsap.context(()=>{
      ...
      gsap.fromTo('.fullscreen-frame',{width:'38vw'},{width:'100vw',ease:'none',scrollTrigger:{trigger:'.fullscreen-moment',start:'top top',end:'bottom bottom',scrub:1,pin:'.fullscreen-frame'}})
      gsap.to('.progress__bar',{scaleY:1,ease:'none',scrollTrigger:{start:0,end:'max',scrub:.2}})
      document.querySelectorAll('.section-theme').forEach(s=>ScrollTrigger.create({trigger:s,start:'top 55%',onEnter:()=>gsap.to(document.body,{backgroundColor:s.dataset.bg,duration:.8}),onEnterBack:()=>gsap.to(document.body,{backgroundColor:s.dataset.bg,duration:.8})}))
      const mm=gsap.matchMedia()
      mm.add('(min-width: 1025px)',()=>{
        ScrollTrigger.create({trigger:'.concept',start:'top top',end:'bottom bottom',pin:'.concept__sticky',pinSpacing:false})
        const track=document.querySelector('.stylist-track');gsap.to(track,{x:()=>-(track.scrollWidth-innerWidth+innerWidth*.12),ease:'none',scrollTrigger:{trigger:'.stylist',start:'top top',end:()=>`+=${track.scrollWidth}`,scrub:1,pin:true,invalidateOnRefresh:true}})
        document.querySelectorAll('.style-tile:nth-child(3n+1),.salon-shot').forEach((el,i)=>gsap.to(el,{y:i%2?-35:35,ease:'none',scrollTrigger:{trigger:el,start:'top bottom',end:'bottom top',scrub:1.3}}))
      })
      return()=>mm.revert()
    },root)
