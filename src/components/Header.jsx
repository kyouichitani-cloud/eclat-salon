...
export default function Header() {
  ...
  return <header className={`header ${scrolled?'header--scrolled':''}`}>
    ...
    <AnimatePresence>{open&&<motion.nav id="mobile-nav" className="mobile-nav" aria-label="モバイルナビゲーション" initial={{opacity:0,y:'-100%'}} animate={{opacity:1,y:0}} exit={{opacity:0,y:'-100%'}} transition={{duration:.55,ease:[.76,0,.24,1]}}>{links.map((x,i)=><motion.a key={x} href={`#${x.toLowerCase()}`} onClick={()=>setOpen(false)} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:.12+i*.035}}>{x}<small>0{i+1}</small></motion.a>)}</motion.nav>}</AnimatePresence>
  </header>
}
